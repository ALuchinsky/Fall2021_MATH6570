...
```{r}
df5 <- readRDS("../data/processed/tab5.rds")
...
```{r}
if(FALSE) {
  sigma2.list <- c( seq(1, 3, 0.2), seq(3.5, 6, 0.5))
  pb <- knitrProgressBar::progress_estimated( length(sigma2.list))
  for( sigma2 in sigma2.list) {
    knitrProgressBar::update_progress(pb)
    df <- calcPower(n = 200, mu2 = 0, p = 0.80, sigma2 = sigma2)
  }
}
```
...
if(FALSE) {
  ...
  for( p in p.list) {
    knitrProgressBar::update_progress(pb)
    df <- calcPower(n = 50, mu2 = 0, sigma2 = 4, p = p)
  }
}
```
...
n_ <- 100
mu2_ <- 0
df %>% filter(n==n_ & mu2 == mu2_) %>% 
ggplot(aes(x=sigma2, y=P, group = p, color = factor(p))) + geom_line() + geom_point() + geom_point() +
  geom_hline(yintercept = 0) + geom_vline(xintercept = 1) + geom_hline(yintercept = 0.05, lty=2) +
  labs(x = "sigma2", title = paste("Figs 5, 6: n=", n_," mu2=", mu2_))
```
...
ggplot(aes(x=sigma2, y=P, group = p, color = factor(p))) + geom_line() + geom_point() + geom_point() +
  geom_hline(yintercept = 0) + geom_vline(xintercept = 1) + geom_hline(yintercept = 0.05, lty=2) +
  labs(x = "sigma2", title = paste("Figs 7, 6: n=", n_," mu2=", mu2_))
```
```{r}
n_ <- 50
mu2_ <- 0
...
```
```{r}
df %>% filter(n==n_ & mu2 == mu2_ & sigma2 == sigma2_ & p == 0.1)
```
